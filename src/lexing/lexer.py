    ...
    def tokenize(self, text: str) -> List[Token]:
        ...
        while index < length:
            ...
            if not token:
                ...
                raise SyntaxError(
                    f"Unexpected character '{text[index]}' at line {self.line}, col {self.column}"
                )
        ...
        while index < len(text) and current_states:
            ...
            for state in current_states:
                if state.is_accepting:
                    ...
                    if (
                        best_match is None
                        or token_length > best_match[2]
                        or (
                            token_length == best_match[2]
                            and state.pattern_index < best_match[3]
                        )
                    ):
                        best_match = (
                            state.token_type,
...
def run_tests():
    ...
    lexer = Lexer([("FLOAT", r"\d+\.\d+"), ("INT", r"\d+"), ("IF", r"if")])
